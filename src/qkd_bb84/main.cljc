(ns qkd-bb84.main
  "Kotodama WASM entrypoint for the qkd_bb84 clean-room actor (L4) — Clojure port.
  ...
  facts (`qkd_bb84.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "qkd_bb84")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"qkd_bb84.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "qkd_bb84-compat" :tier tier :entities entities} 200])
...
(defn start! [] :qkd_bb84-compat/ready)
