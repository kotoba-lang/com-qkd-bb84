(ns qkd-bb84.main-test
  "Contract + behavioral test for the qkd_bb84-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "qkd_bb84-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'qkd-bb84.main-test)]
